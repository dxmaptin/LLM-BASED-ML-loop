#!/usr/bin/env python3
"""Run LOO predictions on 10 NEW questions for a batch of demographics with GPT-5."""
...
if len(sys.argv) < 2:
    print("Usage: python run_batch_demographics.py demographic1 demographic2 ...")
    sys.exit(1)
...
for demographic in DEMOGRAPHICS:
    ...
    # Initialize agents with default model
    print(f"Initializing agents...")
    ir_agent = DataParsingAgent(base_dir, demographic_name=demographic)
    estimator = EstimatorAgent()  # Use default model with improved prompts
    ...
    for i, concept in enumerate(test_concepts, 1):
        ...
        try:
            ...
            print(f"  Running estimator (1 run)...")
...
batch_file = OUTPUT_DIR / f"results_batch_improved_prompts_{batch_name}.csv"
