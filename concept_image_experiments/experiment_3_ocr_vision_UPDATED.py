"""
Experiment 3: OCR + Vision Embeddings (UPDATED - Using GPT-4o Vision)
Combine text extraction with visual feature embeddings to capture:
- Text content (Vision OCR)
...
Act as an OCR system - extract every piece of text you can see, maintaining:
- Exact wording and spelling
- Text positioning/layout where possible
- All visible text including small print, labels, badges, etc.

Output ONLY the raw extracted text, without interpretation or analysis."""
...
def extract_with_ocr_vision(image_path, output_path):
    """
    Extract content using Vision OCR + Vision Embeddings pipeline
    Step 1: Vision OCR for text
    Step 2: Vision model for visual features
    Step 3: LLM synthesis of both
    """
    print(f"  Step 1: Running Vision-based OCR for text extraction...")
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("EXPERIMENT 3: OCR + VISION EMBEDDINGS (UPDATED)\n")
        f.write("="*80 + "\n\n")
        f.write(f"Image: {os.path.basename(image_path)}\n")
        f.write(f"Method: Image → [GPT-4o Vision OCR + Vision Model] → {OPENAI_MODEL} Synthesis\n")
        f.write(f"Note: Using GPT-4o Vision for both OCR and visual analysis\n")
        f.write("="*80 + "\n\n")
...
def run_experiment_3():
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 3: OCR + VISION EMBEDDINGS (UPDATED)")
    print("="*80 + "\n")
