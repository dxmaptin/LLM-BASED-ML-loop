#!/usr/bin/env python3
"""Run LOO predictions on INITIAL 10 questions for a batch of demographics with comprehensive filter."""
...
if len(sys.argv) < 2:
    print("Usage: python run_initial_10_with_filter.py demographic1 demographic2 ...")
    sys.exit(1)
...
# Output folder for initial set results
OUTPUT_DIR = Path("experimental_result_initial_10_with_filter")
...
print("\n" + "=" * 100)
print("RUNNING LOO PREDICTIONS ON INITIAL 10 QUESTIONS WITH COMPREHENSIVE FILTER")
print("=" * 100)
...
for demographic in DEMOGRAPHICS:
    ...
    # Initialize agents with default model (comprehensive filter is in estimator)
    print(f"Initializing agents...")
    ir_agent = DataParsingAgent(base_dir, demographic_name=demographic)
    estimator = EstimatorAgent()  # Has comprehensive filter built-in
    ...
    for i, concept in enumerate(test_concepts, 1):
        ...
        try:
            ...
            # Run estimator (1 run for quick testing)
            print(f"  Running estimator (1 run with comprehensive filter)...")
...
batch_file = OUTPUT_DIR / f"results_initial_filter_{batch_name}.csv"
